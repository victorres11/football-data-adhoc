#!/usr/bin/env python3
"""
Create side-by-side tables for ESPN and CFBD data - using SORTED CFBD data
"""
...
def extract_cfbd_plays(cfbd_data):
    """Extract ALL CFBD play data - now with proper drive/play ordering"""
    plays = []
...
def create_side_by_side_html(espn_plays, cfbd_plays):
    ...
    <head>
        ...
        <title>ESPN vs CFBD - Side-by-Side Data Comparison (SORTED) (401752873)</title>
        <style>
            body {{
    ...
    <body>
        <div class="header">
            <h1>🏈 ESPN vs CFBD - Side-by-Side Data Comparison (SORTED)</h1>
            <h2>Michigan vs Washington - Game ID: 401752873</h2>
            <p>Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
        </div>
        
        <div class="sort-notice">
            <strong>✅ SORTED:</strong> CFBD plays are now sorted by drive number and play number (1.1, 1.2, 1.3, 2.1, 2.2, etc.)
        </div>
    ...
    html += """
            ...
            <div class="table-wrapper">
                <div class="table-header cfbd-header">
                    📊 CFBD Data (SORTED) - All """ + str(len(cfbd_plays)) + """ Plays
                </div>
                <div class="table-container">
                    <table>
                        <thead>
...
def main():
    """Main function to create sorted side-by-side comparison"""
    print("Creating SORTED side-by-side ESPN vs CFBD comparison for game 401752873...")
    ...
    print(f"✅ SORTED side-by-side comparison saved to: {output_file}")
