#!/usr/bin/env python3
"""
Comprehensive extraction and analysis for Illinois vs Purdue game
Game ID: 401752861
Date: October 4, 2025
            ...
            for play in drive['plays']:
                ...
                if yards >= 20:
                    ...
                    if not any(st in play_type.lower() for st in ['punt', 'kickoff', 'field goal']):
                        explosive_plays.append({
                            ...
                            'team': 'Unknown'  # Will need to determine team
                        })
                ...
                if team and down == 3:
                    ...
                    fourth_down_plays.append({
                        ...
                        'description': play.get('text', ''),
                        'yards': play.get('statYardage', 0),
                        'result': 'Conversion' if play.get('end', {}).get('down') == 1 else 'Failed',
                        'team': team
                    })
...
def main():
    ...
    # Extract drive data
    quarter_drives, quarter_scores, running_totals = extract_drive_data(data)
