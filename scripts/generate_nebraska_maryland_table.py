#!/usr/bin/env python3
"""
Generate HTML table with play-by-play data and win probability for Nebraska vs Maryland
"""
...
def load_game_data():
    ...
    with open('data/game_401752867/complete_game_data.json', 'r') as f:
        game_data = json.load(f)
    
    with open('data/game_401752867/teams_data.json', 'r') as f:
...
def generate_html_table(game_data, teams_data, win_prob_lookup):
    ...
    game_name = header.get('name', 'Nebraska vs Maryland')
    game_date = header.get('date', '2025-10-11')
    game_id = header.get('id', '401752867')
...
<head>
    ...
    <title>{game_name} - Play by Play with Win Probability</title>
    <style>
        body {{
...
<body>
    <div class="header">
        <h1>{game_name}</h1>
        <p>Play-by-Play with Win Probability | {game_date} | Game ID: {game_id}</p>
    </div>
...
def main():
    print("Generating Nebraska vs Maryland Play-by-Play HTML Table")
    print("Game ID: 401752867 | Date: October 11, 2025")
    print("=" * 60)
    ...
    # Save HTML file
    import os
    os.makedirs('data/game_401752867', exist_ok=True)
    with open('data/game_401752867/playbyplay_table.html', 'w') as f:
        f.write(html)
    
    print(f"HTML table saved to: data/game_401752867/playbyplay_table.html")
    ...
    print(f"  Match rate: {len(win_prob_lookup)}/{len(plays)} ({len(win_prob_lookup)/len(plays)*100:.1f}%)")
